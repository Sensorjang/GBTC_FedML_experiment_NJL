...
def _data_transforms_cifar10():
    ...
    valid_transform = transforms.Compose([transforms.ToPILImage(),transforms.ToTensor(),transforms.Normalize(MNIST_MEAN, MNIST_STD)])

    return train_transform, valid_transform



def load_mnist_data(datadir, process_id, synthetic_data_url, private_local_data, resize=32, augmentation=True, data_efficient_load=False):
    train_transform, test_transform = _data_transforms_cifar10()

    is_download = True;

    if data_efficient_load:
        mnist_train_ds = MNIST(datadir, train=True, download=True, transform=train_transform)
        mnist_test_ds = MNIST(datadir, train=False, download=True, transform=test_transform)
    else:
        mnist_train_ds = MNIST_truncated(datadir, train=True, download=is_download, transform=train_transform)
        mnist_test_ds = MNIST_truncated(datadir, train=False, download=is_download, transform=test_transform)
...
def partition_data(dataset, datadir, partition, n_nets, alpha, process_id, synthetic_data_url, private_local_data):
    ...
    X_train, y_train, X_test, y_test, cifar10_train_ds, cifar10_test_ds = load_mnist_data(datadir, process_id, synthetic_data_url, private_local_data)
...
def get_dataloader_MNIST(
    ...
    if data_efficient_load:
        dl_obj = MNIST_truncated_WO_reload
        train_ds = dl_obj(
...
def load_partition_data_distributed_mnist(
    process_id,
    dataset,
    data_dir,
    partition_method,
    ...
    if process_id == 0:
        train_data_global, test_data_global = get_dataloader(
            ...
            batch_size,
            data_efficient_load=True,
            full_train_dataset=mnist_train_ds,
            full_test_dataset=mnist_test_ds,
        )
...
def efficient_load_partition_data_mnist(
    args,
    dataset,
    data_dir,
    partition_method,
    ...
    #     test_data_local_dict_mixedl[idx] = test_data_local_dict[client_idx]
    #     idx += 1
    # # NIID可控逻辑^^^^
