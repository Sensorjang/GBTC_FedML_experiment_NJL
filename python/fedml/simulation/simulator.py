...
from ..constants import (
    ...
    FedML_FEDERATED_OPTIMIZER_FEDGBTC,
    FedML_FEDERATED_OPTIMIZER_FEDFARA,
    FedML_FEDERATED_OPTIMIZER_FEDCS,
    FedML_FEDERATED_OPTIMIZER_FAIR,
)
...
class SimulatorSingleProcess:
    def __init__(self, args, device, dataset, model, client_trainer=None, server_aggregator=None):
        ...
        from .sp.fedfara import FedFARAAPI
        from .sp.fedgbtc import FedGBTCAPI
        from .sp.fedcs.fedcs_api import FedCSAPI
        from .sp.fair.fair_api import FairAPI
        ...
        # Mine
        elif args.federated_optimizer == FedML_FEDERATED_OPTIMIZER_FEDGBTC:
            self.fl_trainer = FedGBTCAPI(args, device, dataset, model)
        elif args.federated_optimizer == FedML_FEDERATED_OPTIMIZER_FEDFARA:
            self.fl_trainer = FedFARAAPI(args, device, dataset, model)
        elif args.federated_optimizer == FedML_FEDERATED_OPTIMIZER_FEDCS:
            self.fl_trainer = FedCSAPI(args, device, dataset, model)
        elif args.federated_optimizer == FedML_FEDERATED_OPTIMIZER_FAIR:
            self.fl_trainer = FairAPI(args, device, dataset, model)
