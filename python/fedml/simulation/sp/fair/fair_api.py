...
# FAIR LOSS History
fair_loss_list = [[0 for j in range(global_client_num_in_total)] for i in range(250)]
fair_quality_score_list = [[0 for j in range(global_client_num_in_total)] for i in range(250)]
fair_rou = 0.8
...
# 写入精度指标的标头行
round_ws.append(['Round', 'Loss', 'Accuracy', 'Time', 'Selected Client Indexs', 'Total Selected Client Times', 'total_budget'])
# 创建工作表
...
class FairAPI(object):
    ...
    def train(self):
        ...
        for round_idx in range(self.args.comm_round):
            ...
            print("round_idx: v   ", round_idx)
            # 遍历train_metrics["losses"]和train_metrics["num_samples"]每一项，计算商输出
            print("Clients Train Loss")
            for i in range(len(train_metrics["losses"])):
                print(train_metrics["losses"][i] / train_metrics["num_samples"][i])
                fair_loss_list[round_idx][i] = train_metrics["losses"][i] / train_metrics["num_samples"][i]

            for client_idx in range(self.args.client_num_in_total):
# param 0
                maxloss = 3
                if round_idx == 0:
                    fair_quality_score_list[0][client_idx] = (maxloss - fair_loss_list[0][client_idx]) * train_metrics["num_samples"][client_idx]
                else:
                    molecular = 0
                    denominator = 0
                    # 遍历从round_idx到0
                    for i in range(round_idx, -1, -1):
                        molecular += pow(fair_rou, round_idx - i) * (fair_loss_list[i][client_idx] - fair_loss_list[i-1][client_idx])  * train_metrics["num_samples"][client_idx]
                        denominator += pow(fair_rou, round_idx - i)
                    fair_quality_score_list[round_idx][client_idx] = molecular / denominator
...
# 参数3
            ...
            client_info = []
            for i in range(len(w_locals)):
                client_info.append((client_indexes[i], bid_list[client_indexes[i]], fair_quality_score_list[round_idx][client_indexes[i]]))
                bid_quality_ws.append([round_idx, client_indexes[i], bid_list[client_indexes[i]], fair_quality_score_list[round_idx][client_indexes[i]]])
            # 输出client_info
            ...
            final_selected = []
            removed_clients = []
            start_delete_clents = False
            sum_quality = 0
            for i, (idx, bid, quality_score) in enumerate(client_info):
                sum_quality += quality_score
                if start_delete_clents:
                    removed_clients.append(idx)
                else:
                    budget = sum_quality * bid / quality_score
                    if total_budget >= budget:
                        final_selected.append(idx)
                        # total_budget -= budget
                    else:
                        start_delete_clents = True
                        removed_clients.append(idx)

            # 处理未被选择的客户的删除操作
            removed_clients.sort(reverse=True)
            for i in removed_clients:
                # 移除w_locals第idx个元素
                # print("w_locals.length = " + str(len(w_locals)) + "\n" + "eweights.length = " + str(len(eweights)))
                w_locals.pop(i)
                client_indexes.remove(i)

            print("removed_clients = " + str(removed_clients))
            print("final_selected = " + str(final_selected))

            # 展示最终被选中的client的index
            logging.info("FAIR_final_indexes = " + str(client_indexes))
            ...
            wb.save(self.args.excel_save_path + self.args.model + "_[" + self.args.dataset +"]_fair_training_results_NIID"+ str(self.args.experiment_niid_level) +".xlsx")
    ...
    def _aggregate(self, w_locals):
        ...
        for idx in range(len(w_locals)):
            (sample_num, averaged_params, _) = w_locals[idx]
...
def plot_accuracy_and_loss(self, round_idx):
    ...
    plt.clf()
    plt.suptitle("Fair" + "_[" + self.args.dataset +"]_NIID"+ str(self.args.experiment_niid_level))
    ...
    # 第4个子图，使用折线图展示每个客户的loss
    plt.subplot(2, 3, 4)
    plt.title("clients' loss")
    plt.xlabel("Clients' Indexes")
    plt.ylabel("value of loss")
    for i in range(len(fair_loss_list)):
        plt.scatter(range(1, global_client_num_in_total+1), fair_loss_list[i], label="Round {}".format(i + 1))
    plt.legend()

    # 第5个子图，使用折线图展示每个客户的质量分数
    plt.subplot(2, 3, 5)
    plt.title("clients' quality score")
    plt.xlabel("Clients' Indexes")
    plt.ylabel("value of quality score")
    for i in range(len(fair_quality_score_list)):
        plt.scatter(range(1, global_client_num_in_total+1), fair_quality_score_list[i], label="Round {}".format(i + 1))
    plt.legend()
